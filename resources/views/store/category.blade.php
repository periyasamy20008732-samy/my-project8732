			...
			<div class="page-wrapper-new p-0">
				<div class="content">
					...
					<div class="modal-body custom-modal-body">
						...
						<form action="{{ route('store.category.store') }}" method="POST" enctype="multipart/form-data">
							@csrf
							<div class="mb-3">
							...
							<div class="mb-3">
								<label class="form-label">Category Blug</label>
					...
					<div class="modal-body custom-modal-body">
						...
						<form method="POST" id="editBrandForm" enctype="multipart/form-data">
							...
							<div class="col-lg-12">
								<div class="mb-3 input-blocks">
									<label class="form-label">Description</label>
									<textarea name="description" id="edit_description" class="form-control mb-1"
...
<script>
	$(document).ready(function () {
        ...
        const table = $('#brandTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('store.category.index') }}",
            columns: [
                { data: 'DT_RowIndex', name: 'DT_RowIndex' },
				{
					...
					render: function (data, type, row) {
						if (data) {
							return `<img src="${window.location.origin}/${data}" width="50" height="50" alt="Category Image">`;
						} else {
							return 'No Image';
						}
					},
        ...
        $(document).on('click', '.view-btn', function () {
            ...
            $.ajax({
                url: `/store/category/${packageId}`,
                type: 'GET',
                success: function (data) {
                    $('#view_category_name').text(data.category_name ?? '');
                    $('#view_slug').text(data.slug ?? '');
                   // $('#view_tax').text(data.tax ?? '');
                    $('#view_status').text(data.status == 1 ? 'Active' : 'Inactive');
                if (data.brand_image) {
                    $('#view_category_image').attr('src', '/' + data.category_image).show();
                } else {
                    $('#view_category_image').hide();
                }
        ...
        $(document).on('click', '.edit-btn', function () {
            ...
            $.get(`/store/category/${id}`, function (data) {
                $('#edit_id').val(data.id);
               ...
               // $('#edit_status').prop('checked', data.status === 1 || data.status === '1'); 
                ...
                $('#editBrandForm').attr('action', `/store/category/${data.id}`);
